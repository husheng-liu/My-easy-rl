...
# 
# why does each state is represented by a `[3, 240, 256]` size array?
# 
#   it uses image to represent state. 3 means rgb. 240x256 means image size.
#   why not use ray 
# 
...
if __name__=='__main__':
    ...
    env = gym_super_mario_bros.make("SuperMarioBros-4-2-v3", render_mode='human', apply_api_compatibility=True)
    ...
    # env = RecordVideo(env, video_folder="mario_videos")
    from custom_record_video import CustomRecordVideo
    def record_trigger(episode):
        return episode % 2000 == 0  

    # env = CustomRecordVideo(env, video_folder="mario_videos", record_trigger=record_trigger)
    ...
    for e in range(episodes):
        ...
        while True:
            ...
            if done or info["flag_get"]:
                # env.step(action)
                break
        print(f"total_reards: {total_rewards}")
        logger.log_episode()
        # env.increment_episode()  # 增加episode计数

        if (e % 200 == 0) or (e == episodes - 1):
            logger.record(episode=e, epsilon=mario.exploration_rate, step=mario.curr_step)
            mario.save(save_path=f"episode_{e}.ckpt")
